...
from daipeproject.bronze.tbl_repayments.csv_schema import get_schema as get_csv_schema
...
# MAGIC 1. Edit the `src/daipeproject/_config/config.yaml` file on your local machine
# MAGIC 1. Deploy changes back to Databricks by using the `console dbx:deploy-master-package` command.
# MAGIC
# MAGIC #### Hidden files
# MAGIC
# MAGIC It is also possible to use hidden files for longer reusable parts of code such as schemas. The `daipeproject.bronze.tbl_repayments.csv_schema.py` is used here as an example. Creating these files requires the local developement environment as they are __not visible__ from Databricks.
