...
# MAGIC
# MAGIC ```python
# MAGIC table_schema = dp.TableSchema(full_table_identifier: str, fields: List[t.StructField], primary_key: Union[str, list], partition_by: Union[str, list] = None, tbl_properties: dict = None)
# MAGIC ```
...
# MAGIC The `get_schema()` function should be passed as a second argument into the `@dp.table_overwrite`, `@dp.table_upsert` or `@dp.table_append` decorators.
...
@dp.transformation(dp.read_table("bronze.tbl_loans"), display=True)
@dp.table_overwrite("silver.tbl_loans", get_schema())
