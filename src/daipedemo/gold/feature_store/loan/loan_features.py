...
@transformation(get_unique_observations, get_widget_value("run_date"))
@loan_feature(
    (
        "NewCreditCustomer",
        "Did the customer have prior credit history in Bondora? False = Customer had at least 3 months of credit history in Bondora, "
        + "True = No prior credit history in Bondora",
    ),
    (
        "VerificationType",
        "Method used for loan application data verification 0 Not set 1 Income unverified 2 Income unverified, cross-referenced by phone"
    ...
    (
        ...
        + "101 Working capital financing 102 Purchase of machinery equipment 103 Renovation of real estate 104 Accounts receivable "
        + "financing 105 Acquisition of means of transport 106 Construction finance 107 Acquisition of stocks 108 Acquisition of real "
        + "estate 109 Guaranteeing obligation 110 Other business All codes in format 1XX are for business loans that are not supported "
        + "since October 2012",
    ),
    ...
    (
        ...
        + "15 Civil service & military 16 Education 17 Healthcare and social help 18 Art and entertainment 19 Agriculture, forestry and "
        + "fishing",
    ),
    (
        "HomeOwnershipType",
...
def save_unique_observations_parsed(df: DataFrame, run_date):
    return df.select(*(f.col(c).cast("double").alias(c) if c in numeric_features else f.col(c) for c in df.columns)).withColumn(
        "run_date", f.lit(dt.datetime.strptime(run_date, "%Y-%m-%d"))
    )
